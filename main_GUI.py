...
class NutrtionDataCalculator:
    ...
    def main_calculator(self, ):
        st.title("八大營養標示計算器")
        st.info("此為營養標示計算器，請輸入配方與每份含量克數後點擊計算按鈕，即可顯示八大營養標示結果。（營養成份資料來源於all_data.xlsx檔案）")
        st.header("配方：")
        ...
        if calculate_button:
            df_result1, df_result2 = self.calculate_nutrition_from_excel(edited_df, self.df_nutrition, apiece_weight)
            header1, hesder2 = st.columns(2)
            header1.subheader(f"每份 (含量{apiece_weight}g) 營養標示")
            hesder2.subheader("100 g 營養標示")
            h1, h2 = st.columns(2)
            h1.write()
            h1.dataframe(df_result1)
            h2.dataframe(df_result2)
